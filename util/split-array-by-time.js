...
const timePlot = ['00:00', '00:15', '00:30', '00:45', '01:00', '01:15', '01:30', '01:45', '02:00', '02:15', '02:30',
...
let fn = function (array, timeRange) {
  ...
  timeRange = timeRange.map(item => new Moment(item, 'HH:mm'));
  while (timeRange[0].minute() % 15) {
    timeRange[0].add(1, 'minutes');
  }
  console.log(timeRange[0]);
  while (timeRange[1].minute() % 15) {
    timeRange[1].subtract(1, 'minutes');
  }


  let start = timePlot.indexOf(timeRange[0].format('HH:mm'));
  let end = timePlot.indexOf(timeRange[1].format('HH:mm')) + 1;
